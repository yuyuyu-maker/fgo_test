#!/usr/bin/env bash
# G1 full round: frozen complete baseline teacher + reflow KD + all other ideas.
# 8192 envs, 2000 iters (same budget as the finished G1 baseline).
# Usage: GPU=2 bash scripts/run_g1_all_ideas_teacher_kd.sh
set -eo pipefail
...
export TMPDIR="${TMPDIR:-/dev/shm/_g1_all_ideas_teacher_kd_tmp}"
...
EXP_NAME="g1_all_ideas_teacher_kd"
...
WORKDIR="/dev/shm/g1_all_ideas_teacher_kd"
WS_LOG_DIR="${ROOT}/logs/isaaclab_fpo/g1_all_ideas_teacher_kd_${MAX_ITERS}"
...
echo "[$(date '+%F %T')] G1 ${VARIANT} GPU=${GPU} envs=${NUM_ENVS} iters=${MAX_ITERS}" | tee -a "$OUT"
echo "  teacher=${TEACHER}" | tee -a "$OUT"
echo "  stack=teacher_kd + fpo_operator weights + adaptive_compute + theory (no mix, no ema x1)" | tee -a "$OUT"
...
if [[ -n "$ckpt" && -f "$ckpt" ]]; then
  ...
  CUDA_VISIBLE_DEVICES="${GPU}" python -u "${ROOT}/isaaclab_fpo/scripts/eval_sampling_steps.py" \
    ...
    --model "all_ideas_teacher_kd=${ckpt}" \
