...
## FPO++ (flow policy, same Go2 env)

Gaussian PPO training is `scripts/rsl_rl/train.py`. **FPO++** on the same `Unitree-Go2-Velocity` task is the **baseline** flow learner only (`reflow_enabled=False`). Reflow / reward_aware / … are separate research ideas, not FPO++.

```bash
GPU=1 FPO_VARIANT=baseline bash scripts/run_go2_fpo.sh
# or
./unitree_rl_lab.sh -f --task Unitree-Go2-Velocity --fpo_variant baseline
```

See [docs/FPO_PLUSPLUS.md](docs/FPO_PLUSPLUS.md).
