...
def add_fpo_args(parser: argparse.ArgumentParser):
    ...
    arg_group.add_argument(
        ...
        type=str,
        default=None,
        help="Frozen teacher checkpoint (FPO flow actor, or PPO .pt when teacher_kind=ppo).",
    )
    arg_group.add_argument(
        "--teacher_kd_coef",
        type=float,
        default=None,
        help="Override algorithm.teacher_kd_coef (PPO/flow action distillation weight).",
    )
    arg_group.add_argument(
        "--teacher_aux_zero_x0_prob",
        type=float,
        default=None,
        help="Override algorithm.teacher_aux_zero_x0_prob (aux KD / reflow zero-x0 mix).",
    )
    arg_group.add_argument(
    ...
    arg_group.add_argument(
        ...
        help=(
            "Learner preset. Default / 'baseline' = FPO++ (flow policy, no reflow). "
            "Other names (reflow, reward_aware, …) are extra research ideas on top of FPO++, not FPO++ itself."
        ),
    )
    arg_group.add_argument(
...
def _fpo_variants_for_task(task_name: str):
    ...
    if task_name.startswith("Unitree-G1"):
        from isaaclab_fpo.task_cfgs import UNITREE_G1_FPO_VARIANTS

        return dict(UNITREE_G1_FPO_VARIANTS)
    if task_name.startswith("Unitree-H1"):
        from isaaclab_fpo.task_cfgs import UNITREE_H1_FPO_VARIANTS

        return dict(UNITREE_H1_FPO_VARIANTS)
...
def update_fpo_cfg(agent_cfg: FpoRslRlOnPolicyRunnerCfg, args_cli: argparse.Namespace):
    ...
    if getattr(args_cli, "teacher_kd_coef", None) is not None:
        agent_cfg.algorithm.teacher_kd_coef = float(args_cli.teacher_kd_coef)
    if getattr(args_cli, "teacher_aux_zero_x0_prob", None) is not None:
        agent_cfg.algorithm.teacher_aux_zero_x0_prob = float(args_cli.teacher_aux_zero_x0_prob)
    ...
    # Pin the learner to the same GPU as AppLauncher (--device). Default cfg is cuda:0.
