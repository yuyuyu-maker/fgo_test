...
class ActorCritic(nn.Module):
    ...
    def get_ppo_action_kd_loss(
        ...
        """Match student flow actions to a frozen PPO action (no teacher flow).

        Endpoints / reflow are not involved. ``teacher_actions`` must be detached.
        """
        assert observations.shape[0] == teacher_actions.shape[0]
        batch_size = observations.shape[0]
        device = observations.device
